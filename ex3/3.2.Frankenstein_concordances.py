...
def concordance(lemmatized_tokens, words):
    for word in words:
        print(f"Concordance for {word}")
        nltk_text = nltk.Text(lemmatized_tokens)
...
def main():
    ...
    for i in range(k):
        if (
            top_k_names[i]
            != "Frankenstein; Or, The Modern Prometheus by Mary Wollstonecraft Shelley (65647)"
        ):
            continue
    ...
    concordance(lemmatized_tokens, words)
